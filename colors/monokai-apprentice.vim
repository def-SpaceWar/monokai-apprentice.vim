let g:colors_name='monokai-apprentice'

set cursorline
set cursorlineopt=number
syntax reset
highlight Normal          guifg='#EFEFEF' guibg='#212223' gui=none   ctermfg=15   ctermbg=none  cterm=none
highlight LineNr          guifg='#A0A0A0' guibg=none      gui=none   ctermfg=7    ctermbg=none  cterm=none
highlight CursorLineNr    guifg='#C792EA' guibg='#4A4B4C' gui=bold   ctermfg=5    ctermbg=0     cterm=bold
highlight VertSplit       guifg='#242526' guibg='#4A4B4C' gui=none   ctermfg=0    ctermbg=8     cterm=none
highlight Statement       guifg='#EB3D54' guibg=none      gui=none   ctermfg=1    ctermbg=none  cterm=none
highlight Directory       guifg='#C792EA' guibg=none      gui=none   ctermfg=5    ctermbg=none  cterm=none
highlight StatusLine      guifg='#A0A0A0' guibg='#4A4B4C' gui=none   ctermfg=7    ctermbg=8     cterm=none
highlight StatusLineNC    guifg='#A0A0A0' guibg='#4A4B4C' gui=none   ctermfg=7    ctermbg=8     cterm=none
highlight Comment         guifg='#4A4B4C' guibg=none      gui=italic ctermfg=8    ctermbg=none  cterm=italic
highlight Constant        guifg='#63C485' guibg=none      gui=none   ctermfg=2    ctermbg=none  cterm=none
highlight Special         guifg='#FFCE57' guibg=none      gui=none   ctermfg=3    ctermbg=none  cterm=none
highlight SpecialChar     guifg='#C792EA' guibg=none      gui=none   ctermfg=5    ctermbg=none  cterm=none
highlight Delimiter       guifg='#A0A0A0' guibg=none      gui=none   ctermfg=7    ctermbg=none  cterm=none
highlight Identifier      guifg='#FFCE57' guibg=none      gui=none   ctermfg=3    ctermbg=none  cterm=none
highlight PreProc         guifg='#5A98D9' guibg=none      gui=none   ctermfg=4    ctermbg=none  cterm=none
highlight TSNamespace     guifg='#FFCE57' guibg=none      gui=none   ctermfg=3    ctermbg=none  cterm=none
highlight String          guifg='#63C485' guibg=none      gui=none   ctermfg=2    ctermbg=none  cterm=none
highlight Number          guifg='#63C485' guibg=none      gui=none   ctermfg=2    ctermbg=none  cterm=none
highlight Function        guifg='#5A98D9' guibg=none      gui=none   ctermfg=4    ctermbg=none  cterm=none
highlight Type            guifg='#12CEDE' guibg=none      gui=none   ctermfg=6    ctermbg=none  cterm=none
highlight TSTypeBuiltin   guifg='#EB3D54' guibg=none      gui=none   ctermfg=1    ctermbg=none  cterm=none
highlight Visual          guifg='#212223' guibg='#C792EA' gui=none   ctermfg=16   ctermbg=5     cterm=none
highlight SignColumn      guifg=none      guibg='#4A4B4C' gui=none   ctermfg=none ctermbg=0     cterm=none
highlight Title           guifg='#A0A0A0' guibg=none      gui=bold   ctermfg=7    ctermbg=none  cterm=bold
highlight Pmenu           guifg='#EFEFEF' guibg='#242526' gui=none   ctermfg=15   ctermbg=0     cterm=none
